...
import { useBlockSync } from "@/lib/hooks/useUpdateScript";
...
export const SortableBlock = memo<SortableBlockProps>(
  (props: SortableBlockProps) => {
    ...
    const { mutateAsync } = useBlockSync();
    const params = useParams();
    const { slug: scriptId } = params as Record<string, string | undefined>;
    const handleSync = () => {
      if (scriptId) mutateAsync({ id: scriptId });
    };
    return (
  ...
  (prevProps, nextProps) => {
    ...
    const prevBlockCorrections =
      prevProps.corrections?.filter((c) => c.blockId === prevProps.block.id) ||
      [];
    const nextBlockCorrections =
      nextProps.corrections?.filter((c) => c.blockId === nextProps.block.id) ||
      [];
