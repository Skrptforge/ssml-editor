...
interface CorrectionPanelProps {
  ...
  handleSyncBlocks: () => void;
}

export const CorrectionPanel = memo<CorrectionPanelProps>(
  ({
    corrections,
    blockId,
    onApplyCorrection,
    ...
    const highestSeverity = useMemo(() => {
      ...
      return blockCorrections.reduce((highest, correction) =>
        severityOrder[correction.severity] > severityOrder[highest.severity]
          ? correction
          : highest
      );
    }, [blockCorrections]);
    ...
    const getSeverityColor = (severity: string) => {
      switch (severity) {
        case "high":
          return "text-red-500 bg-red-50 border-red-200 hover:bg-red-100";
        case "medium":
          return "text-orange-500 bg-orange-50 border-orange-200 hover:bg-orange-100";
        case "low":
          return "text-yellow-600 bg-yellow-50 border-yellow-200 hover:bg-yellow-100";
        default:
          return "text-gray-500 bg-gray-50 border-gray-200 hover:bg-gray-100";
      }
    };
    ...
    const handleApplyCorrection = (correction: FactCheckCorrection) => {
      ...
      handleDismissCorrection(correction);
      if (!timeref.current) {
    ...
    const handleDismissCorrection = (
      correctionToRemove: FactCheckCorrection
    ) => {
      // Remove this specific correction from the global store
      const { corrections: allCorrections, actions } =
        useEditorStore.getState();
      const updatedCorrections = allCorrections.filter(
        (c) => c !== correctionToRemove
      );
    ...
    return (
      <div className="flex flex-col gap-2">
        ...
        <div className="flex items-center gap-2 px-6 py-1">
          ...
          >
            {React.createElement(
              getSeverityIcon(highestSeverity?.severity || "low"),
              {
                className: "w-4 h-4",
              }
            )}
            <span className="font-semibold">
              {blockCorrections.length} correction
              {blockCorrections.length > 1 ? "s" : ""}
            </span>
            {showCorrections ? (
  ...
  (prevProps, nextProps) => {
    ...
    const prevBlockCorrections = prevProps.corrections.filter(
      (c) => c.blockId === prevProps.blockId
    );
    const nextBlockCorrections = nextProps.corrections.filter(
      (c) => c.blockId === nextProps.blockId
    );

    return (
      prevProps.blockId === nextProps.blockId &&
      JSON.stringify(prevBlockCorrections) ===
        JSON.stringify(nextBlockCorrections)
    );
  }
);
